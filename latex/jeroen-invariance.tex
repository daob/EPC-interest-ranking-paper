...
\begin{abstract}
Ranking data are often encountered in political science, for example when modeling voters' choices in ``ranked choice'' voting systems or their priorities for political goals. When comparing different groups on such measures, a concern is whether such comparisons can be said to be unaffected by any differences in measurement. However,  there currently is no systematic way to examine this hypothesis for ranking data. This paper therefore extends the ``$\da$'' a measure recently introduced for that purpose in this journal, to the case of latent variable models with ranking data. We demonstrate the approach on a multilevel latent class analysis of three ``(post)materialism'' measures using data from 67,568 respondents in 48 different countries. 
