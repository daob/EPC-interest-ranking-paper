...
Ranking data are central to political science, for example when  citizens vote by ranking candidates or rank their priorities on widely-used survey measures of (post)materialism. Such procedures yield categorical data that preclude certain joint outcomes, leading to complex dependencies that must be accounted for. Moreover, when the results of ranking tasks are compared across groups, not only measurement error, but also ``measurement equivalence''  becomes a concern:  conclusions of substantive interest should be uncontaminated by measurement differences.

In this study, we apply a measure recently introduced in this journal, the ``EPC-interest'', to a  multilevel latent variable model for ranking data. The EPC-interest evaluates directly whether conclusions of substantive interest are sensitive to the problem of measurement differences. The approach is applied to 48 countries and 67,568 respondents to the 2010--2012 World Values Survey; interest focuses on the widely-used (post)materialism measures and their relationship with economic and social correlates. The contributions of this paper are threefold: first, we allow for measurement error in the WVS (post)materialism ranking tasks; second, we use the EPC-interest to evaluate the practical impact of measurement non-equivalence on analyses of central interest to (post)materialism values researchers; and third, we extend the EPC-interest measure to models with discrete observed and latent variables. 
...
% Our model can be seen as a multilevel multinomial regression of (post)materialism on country-level covariates, in which the nominal dependent variable is latent (corrected for misclassification), and the random effects distribution is nonparametric rather than Gaussian. The measurement process for this latent nominal dependent variable accounts for the intrinsic dependencies of ranking data.
...
%Ik weet niet of McFadden het ranking model heeft voorgesteld. Volgens mij het heeft hij "enkel" het model voor first choice voorgesteld. Je kunt volgens mij niet zeggen dat aanamen worden "relaxed", ze worden immers vervangen door andere (namelijk van onafhankelijke extreme value verdelingen). Wel wordt het daarmee allemaal een stuk simpler.
%Dat sequentiele ranking model wordt ook wel exploded logit genoemd.
% Croon
% Francis
% Kamakura
...
Probabilistic models with rankings as outcomes that account for these characteristics were introduced by \citet{thurstone1927law}, and extended by \citet{luce1959individual} and \citet{mcfadden1974conditional}. These approaches treat the choices as perfectly measured; however, rankings may contain measurement error, so that latent variable models become necessary. %For example, a respondent's high priority for ``protecting freedom of speech'' may not be a perfect measure of their postmaterialism. 
 \citet{croon1989latent} introduced a latent class model for ranking data while \citet{kamakura1994concomitant} extended this model to include covariates relating to class membership. Other approaches to modeling ranking outcomes with latent variables were discussed by \citet{jackson1980factor}, \citet{francis2002analysing} and \citet{maydeu2005structural}.
...
This may lead a researcher to erroneously conclude that Costa Ricans are less materialist than others, when in fact the apparent difference could be partly due to measurement differences. Similarly, when comparing countries with different percentages of women in parliament, the country with the higher percentage\footnote{See Section 2 for data sources.}, Costa Rica (39\%), may erroneously appear to have fewer ``materialists'' than Russia, the country with the lower percentage (14\%). %Some Costa Ricans could, for instance, prioritize the ``postmaterialist'' goal of ``keeping our countryside beautiful'', but for the ``materialist'' reason of promoting ecotourism.
Thus, measurement differences may contaminate conclusions about both mean differences and relationships between variables.

 This concern of whether groups are comparable in measurement is often called ``measurement equivalence'' or ``measurement invariance'' \citep[see][for reviews]{steenkamp_assessing_1998,vandenberg2000review,schmitt2008measurement}.
...
this concern in terms of the substantive parameters of interest, the ``EPC-interest''. To use this procedure, the researcher initially fits a model that assumes full measurement invariance. The $\da{}$ then estimates the expected change in the parameters of interest that would occur if certain assumptions of measurement invariance were relaxed. To continue the previous example, $\da{}$ might indicate how much of the substantive mean difference in  ``materialism'' between Costa Rica and Russia is contaminated by possible measurement differences, and whether this contamination is capable of reversing the conclusions. While useful, currently $\da{}$ is only available for linear structural equation models for continuous data. Categorical observed and latent variables are, however, commonly found in the social sciences. Ranking data, moreover, have never been examined for cross-group equivalence in the sense that parameters of interest are unaffected by possible measurement differences.


This paper therefore develops the $\da{}$ in more generality. This allows for application of the $\da{}$ approach to measurement equivalence evaluation in, for example, IRT analysis \citep[see also][]{Oberski:WP:model-based-GOF}, ordinal factor analysis, latent class analysis, and to models for ranking data. 
We use an example comparison of the \citet{inglehart1997modernization} (post)materialism ranking measures over 48 countries to demonstrate the application of the $\da{}$ to such data. To do so, we relate each country's GDP per capita and percentage of women in parliament to the country proportion in categories of ``(post)materialism'' as measured by three separate ranking tasks. However, the measurement of these (post)materialism categories may be flawed, so that (post)materialism becomes a categorical latent variable: a ``latent class'' variable \citep{bockenholt2002comparison,moors2007heterogeneity}. The measurement model for this variable accounts for the intrinsic dependencies between respondents' ranking choices, while the structural model accounts for dependencies between respondents within a country using a multilevel specification with a nonparametric random effect \citep{vermunt2003multilevel}. The $\da{}$ is then applied to investigate the impact of measurement invariance assumptions on the latent relationship estimates of interest.
...
In their previous work, Inglehart and colleagues examined these relationships of interest between value priorities and country-level covariates by creating an observed categorization of the ranking choices into three groups:  ``materialist'', ``postmaterialist'', and ``mixed''. These groups are determined by whether the first and second choice on the second ranking set are both ``materialist'', ``postmaterialist'', or a combination of the two.
 Thus, these groups are defined \emph{a priori}, and their usage as observed variables implies that a complete absence of measurement error is assumed. 
...
This dependency intrinsic to ranking tasks makes the model used here different from standard models for categorical data such as latent class or ordinal factor analysis models.
...
For this purpose we  extended the  EPC-interest  from the linear structural equation modeling framework to apply to categorical latent and observed variables and implemented in user-friendly commercial software for latent class analysis. Program input is available in the online Appendix. 
