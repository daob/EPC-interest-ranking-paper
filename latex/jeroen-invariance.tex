...
\title{Evaluating the impact of measurement non-equivalence on the relationship of  (post)materialism with country-level correlates}
\date{}
...
\begin{abstract}
Ranking and paired comparisons data are central to political science, for example when  citizens vote by ranking candidates or rank their priorities on widely-used survey measures of (post)materialism. Such procedures yield categorical data that preclude certain joint outcomes, leading to complex dependencies that must be accounted for. Moreover, when the results of ranking tasks are compared across groups, not only measurement error, but also ``measurement equivalence''  becomes a concern:  conclusions of substantive interest should be uncontaminated by measurement differences.

In this study, we apply a measure recently introduced in this journal, the ``EPC-interest'', to a nonparametric multilevel latent variable model for ranking data. The EPC-interest evaluates directly whether conclusions of substantive interest are sensitive to the problem of measurement differences. The approach is applied to 48 countries and 67,568 respondents to the 2010--2012 World Values Survey; interest focuses on the widely-used (post)materialism measures and their relationship with economic and social correlates. The contributions of this paper are threefold: first, we allow for measurement error in the WVS (post)materialism ranking tasks; second, we use the EPC-interest to evaluate the practical impact of measurement non-equivalence on analyses of central interest to (post)materialism values researchers; and third, we extend the EPC-interest measure to models with discrete observed and latent variables. 
...
Section \ref{sec:data} presents the data and the postmaterialism theory motivating our model. Since the obtained measures are rankings, Section \ref{sec:model} then presents the multilevel latent class model for partial ranking or paired comparisons data. We then present the EPC-interest for such and other latent variable models in Section \ref{sec:epc-interest}, after which the results of applying the EPC-interest to the postmaterialism model and data is discussed in Section \ref{sec:results}. Finally, Section \ref{sec:discussion} provides some conclusions and discusses limitations and connections with other work.
...
We employ the 2010--2012 World Values Survey\footnote{\url{http://www.worldvaluessurvey.org/}} (WVS) comprising $n = 67^,568$ respondents in 48 different countries (Appendix \ref{sec:countries-included} provides a full list of countries).
The WVS questionnaire includes \citet{inglehart1977silent}'s extended post-materialism questions, developed to measure political values priorities. This extended version includes three sets of four priorities (Table \ref{tab:ranking-question}). Of these, set B in Table \ref{tab:ranking-question} is known as the ``short scale'' that is commonly used in research on values priorities.  Two of the values in each set are intended to measure  ``materialist'' concerns while the two remaining priorities indicate ``postmaterialist'' political goals; for each of the   three sets, WVS respondents are then asked to rank their first and second priorities. 
...
Thus, when reading each row horizontally, the class with the highest log-utility represents respondents who value that priority highest. For example, priorities A.1, A.2, B.1, B.3, and C1 have the highest log-utilities in Class 1. Since all of these priorities are ``materialist'' (labeled ``M'' in Table \ref{tab:attribute-parameters}), we also labeled Class 1 ``materialist''. A caveat with this label is that the materialist priorities that are most strongly related to this class also happen to be the first item in each set, so that a primacy effect may play a role here as well.  Class 2 is labeled ``postmaterialist'' because it has the highest log-utilities for all of the postmaterialist priorities (labeled ``P'' in Table \ref{tab:attribute-parameters}), with the exception of A.4. Preferences in the third class appear to be for the most part in-between those of Classes 1 and 2. At the same time, however, this class has the highest log-utilities for A.4 (a ``postmaterialist'' priority) and C.4 (a ``materialist'' priority). For this reason we apply the label ``mixed'' to Class 3.
