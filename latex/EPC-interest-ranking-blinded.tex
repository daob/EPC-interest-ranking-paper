...
In short, some form of measurement invariance is needed to estimate the structural model of interest: deciding which form, that is, selecting a model, is therefore crucial. Currently, there are broadly four existing approaches to doing so:
\begin{enumerate}
	\item Selecting one indicator as the reference indicator or ``anchor item'' \emph{a priori};
		\item Imposing a strong prior on the differential functioning parameters;
	\item Testing the null hypothesis of full measurement invariance;
	\item Evaluating the size and statistical significance of the differential functioning parameters  to obtain a partially invariant model.
\end{enumerate}

Each of these four procedures has its merits and drawbacks.


\emph{A priori} reference indicators are often selected implicitly. In multiple group confirmatory factor analysis (MGCFA), for example, when one loading is fixed to unity in each group, the corresponding indicator is implicitly assumed to be measurement invariant over groups \citep{hancock2009tenuousness}. Recently, \citet{muthen2014alignment} refined this approach by suggesting to rotate the \emph{a priori} reference indicator MGCFA solution towards the fully invariant solution. A disadvantage of the \emph{a priori} approach, however, is that  all models choosing exactly one \emph{a priori} single reference indicator per latent variable and group will fit the data equally well (are equivalent). Thus, it is impossible to select the reference indicator based on the data \citep{hancock2009tenuousness}. 

Recently, \citet{muthen2012bayesian} introduced a Bayesian approach to dealing with lack of measurement invariance: estimate a single model in which all the ``direct effects'' are allowed to differ somewhat from zero by imposing a strongly informative  prior on them. When the prior is chosen adequately, this allows for a modest level of violation of measurement invariance while still identifying the structural model. For this reason \citet{muthen2012bayesian} termed this approach ``approximate measurement invariance''. This new approach is promising, but has some drawbacks as well:  

An key issue with this approach, however, is that of choosing an adequately informative prior. At current, this issue remains unresolved, although future research may bring more clarification. 


Testing for full measurement invariance is done by comparing a full measurement invariance model is compared with a model imposing no invariance besides an \emph{a priori} reference indicator \citep{steenkamp_assessing_1998,french2006confirmatory}.  If full measurement invariance  is not rejected, the structural relationships of interest is then estimated under this  model. Criteria used to accept or reject full measurement invariance include likelihood ratio (chi square) testing, information criteria such as AIC and BIC, and fit measures such as CFI, RMSEA, and gamma-hat \citep{hu1998fit,cheung2002evaluating,chen2007sensitivity}. 
This approach has three serious disadvantages, however. First, the misfit may be due to only a few misspecifications, which could be accounted for in a partially invariant model \citep{byrne1989testing}.
Second, when the full invariance model fits badly, this need not have a strong substantive impact on the parameter of interest \citep{saris2009testing}. And third, when the full invariance model fits well, \citet{Oberski:WP:EPC-interest} showed that there may still be violations of invariance in certain items that substantially bias the parameters of interest.

	
Evaluating size and significance of the direct effect solves the first of these problems by essentially performing a series of tests like the ones described for each possible misspecification, $\delta_{jk} \neq 0$, separately. The size of the direct effect $\delta_{jk}$ may be assessed by estimating $J$ separate models or by use of the ``expected parameter change'' (EPC). After fitting a full invariance model, the EPC estimates the size of $\delta_{jk}$ itself if that parameter were freed \citep{saris_detection_1987}. For this reason, we will refer to this well-known measure as the ``EPC-self''. 
While this approach solves the first problem with omnibus measurement invariance testing, it suffers from the second and third problems, just as omnibus testing does. That is, even when significant and/or large misspecification are found these do not necessarily have a strong substantive impact on the parameters of interest; and, conversely, when the misspecification appears to be small in terms of, for instance, EPC-self, its effect on the parameters of interest may still be considerable \citep{Oberski:WP:EPC-interest}. In short, the problem with measurement invariance testing is that does not account for the effect that violations of measurement invariance have on the parameters of interest.

The EPC-interest was recently introduced by \citet{Oberski:WP:EPC-interest} for continuous data structural equation models. Similar to the EPC-self, it assesses what would happen if a particular possible direct effect were freed. Rather than assessing the size of this direct effect itself, however, it assesses its impact on the parameter of interest. However, this measure is only available for continuous data, whereas many important measures of latent variables in the social sciences are categorical. Examples include the votes (Yea/Nay) of senators indicating their ideology, how respondents rank their value priorities (1 through 4), or answers (correct/incorrect) to political knowledge questions. The following section therefore explains the rationale of the EPC-interest in more detail, extending it to the case of categorical indicators.
...
$$
	P(Y_1 = 1 | x) = [1 + \exp(-x - \delta_1 z)]^{-1}.
$$
Nine conditions varied the following experimental factors:
\begin{itemize}	
	\item Sample size: $n=$ 250, 500, or 1000;
	\item The size of the invariance violation:  $\delta = $ 0 (no violation), 0.5 (moderate), or 1 (extreme). 
\end{itemize}

Data were generated using R 3.1.2 and analyzed using Latent Gold 5.0.0.14161. The results over 200 samples under the nine conditions are shown in Table \ref{tab:simulation}. The first two rows of this table show the average point estimate of the parameter of interest $\hat{\gamma}$, and its deviation from the true value $\gamma = 1$, respectively. It can be seen that a modest violation $\delta = 0.5$ can still have a substantively important impact on bias. Under this condition, that bias is reasonably well approximated by the EPC-interest, which has average values close to the true bias. When the violation is extreme, however, the approximation causes the EPC-interest to somewhat overestimate the bias: for example, in the last column of the table the true bias is -0.327 but EPC-interest estimates it at about -0.448. This effect appears to be strongest with the smallest sample size, a phenomenon that can be explained by the approximation term being more likely to take on large values in small samples. 
...
True $\delta_1$	&   0	& 0.5	& 1	&& 0	& 0.5	& 1	&& 0	& 0.5	& 1\\
	\hline
